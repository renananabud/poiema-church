...
// -----------------------------------------------------
//fazer as cidades aumentarem e mudarem a cor ao passar o mouse por cima

var cidades = $('.div-cidades');
var infoCidades = cidades.children('.div-info-cidade');
var nomeCidade = cidades.children('.nomeCidade');

console.log(nomeCidade);

for(let i = 0; i < cidades.length; i++){
	$(cidades[i]).mouseover(function (){
		for (var i = 0; i < cidades.length; i++ ){
	 		if(cidades[i] != this) {
	 			$(cidades[i]).css("filter", "grayscale(100%)");
	 		}
	 		$(this).children('.div-info-cidade').css("display", "block");
	 		$(this).children('.nomeCidade').css("top", "75px");
	  }
	});

	$(cidades[i]).mouseout(function (){
	 for (var i = 0; i < cidades.length; i++ ){
	 		$(cidades[i]).css("filter", "grayscale(0)");
	 		$(this).children('.div-info-cidade').css("display", "none");
	 		$(this).children('.nomeCidade').css("top", "initial");
